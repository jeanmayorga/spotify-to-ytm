...
export function Player({ token }: Props) {
  ...
  return (
    <>
      {deviceId && (
        <div className="relative bg-black border border-gray-800 overflow-hidden rounded-xl">
            ...
            {currentTrack?.artists && currentTrack?.artists?.length > 0 && (
              <div className="text-gray-300 text-base drop-shadow-md mb-6">
                {currentTrack?.artists.map((artist) => (
                  <Link
                    href={`/spotify/artists/${artist.uri.split("artist:")[1]}`}
      ...
      {devices.length > 0 && (
        ...
        >
          <SelectTrigger className="w-full rounded-xl my-8">
